...
import kw.data.dao.HiveLineageDao;
...
public class LineageHook  implements ExecuteWithHookContext {
    ...
    public void run(HookContext hookContext) throws Exception {
        ...
        try {
            ...
            if (value != null ){
                LOG.info(" hive hook explain sql : " + value);
            }
        }catch (Exception e){
    ...
    private Optional<Lineage> buildLineage(HookContext hookContext) throws SQLException {
        ...
        System.out.println(" get hive hooks lineage :  "+lineage.toString());
        LineageInfo lineageInfo = hookContext.getLinfo();
        System.out.println("lineageInfo :  " + hookContext.getLinfo().entrySet());
        if(lineageInfo == null) {
            return Optional.empty();
        }
        ...
        for (Map.Entry<LineageInfo.DependencyKey, LineageInfo.Dependency> entry : lineageInfo.entrySet()) {
            ...
            if(dep != null) {
                System.out.println("Upstream:   " +"is not null" );
                for (LineageInfo.BaseColumnInfo baseCol : dep.getBaseCols()) {
                    if(baseCol.getColumn() == null){
                        System.out.println("Upstream" +baseCol.toString() +" has NULL FieldSchema");
...
//        HiveLineageDao hd= new HiveLineageDao();
//        if(quesy_sql.toLowerCase().contains("insert")){
//            System.out.println("-----------INSERT ---");
//            List<Table> source=lineage.getSourceTables();
//            Table target=lineage.getTargetTables().get(0);
//            for (Table tab :source){
//                HiveLineage hl=new HiveLineage();
//                hl.setDb(tab.getDbName());
//                hl.setTname(tab.getTableName());
//                hl.setCol(tab.getColumns().toString());
//                hl.setParent_db(target.getDbName());
//                hl.setParent_tname(target.getTableName());
//                hl.setParent_col(target.getColumns().toString());
//                hl.setExpr(quesy_sql);
//                hl.setPtype("INSERT");
